...
### Follow-up features delivered in alpha.5

The proposed semantic hooks, immutable derived clients, total operation deadlines and path-aware schema diagnostics are now implemented. See [semantic DX](semantic-dx.md). Their hooks are configured on the Questions client, separate from the HTTP hooks described above. Providers and retry settings now also accept `timeout`, `initialDelay`, `maxDelay` and `delay` with typed duration strings; the numeric `*Ms` spellings shown in older examples remain compatible.

Offline record/replay remains a future capability, **not implemented**. It requires versioned request/schema/provider identities, bounded storage, redaction, strict matching and revalidation with no silent network fallback. `Execution.replay()` still means a new potentially paid inference. The two must not be conflated.
