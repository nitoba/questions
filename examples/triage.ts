...
const q = liveClient().about({ title: "Production API rejects all requests after credential rotation", retries: 3 });
const result = await q.ask({
  blocked: "Is production work blocked?",
  team: Question.choice("Which team should investigate?", { billing: "Invoices and payments", platform: "API and deployment failures" }),
  impact: Question.score("How disruptive is this?", ["Low", "Work impaired", "Production unavailable"]),
});
console.log(result);
try {
  const owner = await q.branch("Which response is appropriate?", {
