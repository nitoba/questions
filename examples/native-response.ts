...
export function triageResponse(
  ...
  tickets: AsyncIterable<string>,
  signal: AbortSignal,
): Response {
  const bytes = Streams.from(tickets)
    .map(
      async (ticket, { signal: itemSignal, index }) => ({
        index,
        urgent: await client
          .about(ticket)
          .is("Does this require urgent attention?", { signal: itemSignal }),
      }),
      { concurrency: 4 },
    )
    .through(
      () =>
        new TransformStream<{ index: number; urgent: boolean }, Uint8Array>({
