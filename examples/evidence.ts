...
const result = await liveClient()
  .about("Duplicate debit reported, but invoice references differ")
  .evidence({
    category: Question.choice("Which situation best fits?", {
      duplicate: "A duplicated payment",
      legitimate: "Two distinct purchases",
    }),
  });
const evidence = result.answers.category;
console.log({
  ranked: Answer.rank(evidence),
  separation: Answer.margin(evidence),
  usage: result.usage,
});
