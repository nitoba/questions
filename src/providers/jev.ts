...
function requestBody(request: EvaluationRequest, model: string): string {
  ...
  const wire = Object.fromEntries(Object.entries(questions).map(([key, question]) => {
    ...
    switch (question.type) {
      case "boolean": return [key, { type: "noul", instructions: question.instructions,
        ...(question.criteria === undefined ? {} : { criteria: {
          true: describe(question.criteria.true), false: describe(question.criteria.false),
        } }),
      }];
      case "choice": return [key, { ...question,
        criteria: Object.fromEntries(Object.entries(question.criteria).map(([id, value]) => [id, describe(value)])),
      }];
      case "score": return [key, question];
    }
  }));
...
async function readJson(response: Response, limit: number, signal: AbortSignal): Promise<unknown> {
  ...
  try {
    ...
    while (true) {
      ...
      if (bytes > limit) throw new ProviderError("Jev", "response", "Jev response exceeds maxResponseBytes");
    ...
    try { await reader.cancel(); } catch { /* Preserve primary failure. */ }
    reader.releaseLock();
  }
}
...
function normalizeResponse(value: unknown): unknown {
  ...
  return {
    ...
    answers: Object.fromEntries(Object.entries(answers).map(([key, raw]) => {
      ...
      return [key, answer.type === "noul" ? { type: "boolean", probability: answer.noul } : answer];
    })),
  };
}
...
export function create(options: Options): QuestionModel {
  ...
  if (!["https:", "http:"].includes(endpoint.protocol) || endpoint.username || endpoint.password || endpoint.search || endpoint.hash) {
    throw new ValidationError("baseUrl must be an HTTP(S) URL without credentials, query or fragment", "baseUrl");
  }
  ...
  if (maxRetries > 10) throw new ValidationError("at most 10 retries are allowed", "retry.maxRetries");
  ...
  if (maxDelay > 2_147_483_647) throw new ValidationError("delay exceeds platform timer limit", "retry.maxDelayMs");
        ...
        for (let attempt = 0; ; attempt++) {
          ...
          try {
            const pending = fetcher(new URL(endpoint), {
              ...
              body, signal: scope.signal, redirect: "error",
            });
            response = await abortable(Promise.resolve(pending).then((value) => {
          ...
          if (response.ok) return normalizeResponse(await readJson(response, maxResponseBytes, scope.signal));
          const error = new ProviderError("Jev", "http", `Jev returned HTTP ${response.status}`, { status: response.status });
          const requestedDelay = retryAfter(response);
          try { await response.body?.cancel(); } catch { /* Preserve status error. */ }
          if (![429, 529].includes(response.status) || attempt >= maxRetries ||
            (requestedDelay !== undefined && requestedDelay > maxDelay)) throw error;
