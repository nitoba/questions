...
export function sourceStream<T>(source: Source<T>, signal: AbortSignal): ReadableStream<T> {
  ...
  const iterator: Iterator<T> | AsyncIterator<T> = typeof (source as AsyncIterable<T>)[Symbol.asyncIterator] === "function"
    ? (source as AsyncIterable<T>)[Symbol.asyncIterator]() : (source as Iterable<T>)[Symbol.iterator]();
  let ended = false;
  const stop = () => {
    if (ended) return;
    ended = true;
    ...
    catch { /* Do not replace the primary error with an iterator cleanup error. */ }
  };
  return new ReadableStream<T>({
    async pull(controller) {
      try {
...
interface Slot<T> { ready: boolean; value?: T }
...
export function concurrent<T, R>(
  ...
  const cleanup = (reason?: unknown): Promise<void> => {
    ...
    cleanupPromise = (async () => {
      if (reader) {
        try { await reader.cancel(reason); } catch { /* Preserve primary error. */ }
        finally { reader.releaseLock(); }
      }
    })();
  ...
  const pump = async () => {
    ...
    try {
      ...
      while (!ended && !inputDone && slots.size < concurrency) {
        ...
        if (ended) break;
        if (item.done) { inputDone = true; notify(); break; }
        const index = issued++;
  ...
  return new ReadableStream<R>({
    ...
    async pull(controller) {
      ...
      while (!ended) {
        ...
        if (slot?.ready) {
          slots.delete(key!);
          if (ordered) nextOutput++; else ready.shift();
