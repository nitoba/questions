...
function countingSource() {
  ...
  const source = new ReadableStream<number>({
    pull(controller) { controller.enqueue(pulled++); },
    cancel() { canceled++; },
  }, { highWaterMark: 0 });
  return { source, get pulled() { return pulled; }, get canceled() { return canceled; } };
}

test("construction and toReadable do not pull; take(0) does not acquire", async () => {
  let opened = 0;
  const flow = Streams.defer(() => { opened++; return [1, 2]; }).map((n) => n * 2);
...
test("filter, tap and batches preserve order and flush partial batch", async () => {
  const seen: number[] = [];
  const flow = Streams.from([0, 1, 2, 3, 4, 5]).filter(async (n) => n % 2 === 1)
    .tap((n) => { seen.push(n); }).batch(2);
...
test("takeUntil includes matching item; takeWhile excludes it", async () => {
  assert.deepEqual(await Streams.from([1, 2, 3, 4]).takeUntil(async (n) => n === 3).toArray(), [1, 2, 3]);
  assert.deepEqual(await Streams.from([1, 2, 3, 4]).takeWhile(async (n) => n < 3).toArray(), [1, 2]);
...
test("bounded map counts completed outputs, not only running tasks", async () => {
  ...
  const reader = Streams.from(tracked.source).map((n) => {
    started.push(n); return pending[n]!.promise;
  }, { concurrency: 3 }).toReadable().getReader();
  const first = reader.read();
  await tick();
  assert.deepEqual(started, [0, 1, 2]);
  pending[1]!.resolve(11); pending[2]!.resolve(12);
  await tick();
...
test("unordered map emits completion order and preserves undefined outputs", async () => {
  const a = deferred<number | undefined>(); const b = deferred<number | undefined>();
  const reader = Streams.from([a, b]).map((d) => d.promise, { concurrency: 2, ordered: false })
    .toReadable().getReader();
  const first = reader.read();
  await tick(); b.resolve(undefined);
...
test("later ordered rejection fails promptly and aborts noncooperative sibling", async () => {
  ...
  const result = Streams.from([0, 1]).map((n, { signal }) => {
    if (n === 0) { sibling = signal; return new Promise<number>(() => {}); }
    throw failure;
...
test("external abort settles pending map and cancels source", async () => {
  const tracked = countingSource(); const controller = new AbortController();
  const reason = new Error("stop"); let callbackSignal: AbortSignal | undefined;
  const result = Streams.from(tracked.source).map((_, { signal }) => {
    callbackSignal = signal; return new Promise<number>(() => {});
  }).toArray({ signal: controller.signal });
  await tick(); controller.abort(reason);
...
test("preaborted consumption never acquires a source", async () => {
  let acquired = false; const controller = new AbortController(); controller.abort();
  await assert.rejects(Streams.defer(() => { acquired = true; return [1]; }).toArray({ signal: controller.signal }));
...
test("late-acquired native sources are canceled after abort", async () => {
  ...
  const tracked = countingSource(); d.resolve(tracked.source); await tick();
  assert.equal(tracked.canceled, 1);
  assert.equal(tracked.source.locked, false);
});
...
test("native source errors release locks", async () => {
  ...
  const source = new ReadableStream<number>({ pull(controller) { controller.error(failure); } });
...
test("failed predicates release source and preserve failure", async () => {
  const failure = new Error("predicate failed"); const tracked = countingSource();
  await assert.rejects(Streams.from(tracked.source).takeUntil(() => { throw failure; }).toArray(), (error) => error === failure);
  await tick();
  assert.equal(tracked.source.locked, false);
});
...
test("async iteration break finalizes a synchronous generator", async () => {
  let finalized = 0;
  function* items() { try { yield 1; yield 2; } finally { finalized++; } }
  for await (const item of Streams.from(items())) { assert.equal(item, 1); break; }
  assert.equal(finalized, 1);
});

test("single-use generator/native inputs reject a second run; arrays repeat", async () => {
  function* items() { yield 1; }
  const flow = Streams.from(items()); assert.deepEqual(await flow.toArray(), [1]);
  await assert.rejects(flow.toArray(), ValidationError);
  const native = Streams.from(new ReadableStream<number>({ start(c) { c.enqueue(1); c.close(); } }));
...
test("iterable factories are acquired exactly once per execution", async () => {
  let factories = 0;
  const source = { [Symbol.iterator]() { factories++; return [1, 2][Symbol.iterator](); } };
...
test("callback failures and maxItems cancel upstream", async () => {
  const tracked = countingSource(); const failure = new Error("consumer");
  await assert.rejects(Streams.from(tracked.source).forEach(() => { throw failure; }), (error) => error === failure);
  assert.equal(tracked.source.locked, false);
...
test("native transforms and WritableStream interoperate without sharing transform locks", async () => {
  const pipeline = Streams.from([1, 2]).through(() => new TransformStream<number, string>({
    transform(n, c) { c.enqueue(String(n)); },
  }));
  assert.deepEqual(await pipeline.toArray(), ["1", "2"]);
  const values: string[] = [];
  await pipeline.pipeTo(new WritableStream({ write(value) { values.push(value); } }));
  assert.deepEqual(values, ["1", "2"]);
});

test("destination failure cancels source", async () => {
  const tracked = countingSource(); const failure = new Error("destination");
  await assert.rejects(Streams.from(tracked.source).pipeTo(new WritableStream({ write() { throw failure; } })),
    (error) => error === failure);
  assert.equal(tracked.source.locked, false);
});
...
test("invalid stream options are rejected synchronously", () => {
  ...
  for (const concurrency of [0, -1, 1.1, Infinity, NaN]) assert.throws(() => flow.map((n) => n, { concurrency }), ValidationError);
...
test("iterator read failures call return for cleanup", async () => {
  let closed = 0;
  const source: Iterable<number> = { [Symbol.iterator]() {
    return { next() { throw new Error("read"); }, return() { closed++; return { done: true, value: undefined }; } };
  } };
