...
test("live context is evaluated anew; previous promises are not replayed", async () => {
  ...
  assert.deepEqual(calls.map((call) => call.state), [{ revision: 1 }, { revision: 2 }]);
});
...
test("branch runs only the selected handler and preserves sync/async return values", async () => {
  ...
  const result = await q.branch("Route?", {
    billing: () => { first++; return { invoice: 1 }; },
    support: async () => { second++; return { case: 2 }; },
  });
...
test("uncertain branches run no handlers and handler errors are not replaced", async () => {
  let invoked = 0;
  const uncertain = Questions.create(fixture(() => ({
    type: "choice", choice: "a", probabilities: { a: 0.5, b: 0.5 }, confidence: 0,
  }))).about("x");
  await assert.rejects(uncertain.branch("Which?", { a: () => invoked++, b: () => invoked++ }, { confidence: 0.5 }), UncertainDecision);
  assert.equal(invoked, 0);
  const failure = new Error("handler failure");
  await assert.rejects(Questions.create(fixture()).about("x").branch("Which?", {
    a: () => { throw failure; }, b: () => 0,
  }), (error) => error === failure);
});
...
test("each batches all items and safely regroups arbitrary user keys", async () => {
  const { model, calls } = fixture();
  const rows = await Questions.create({ model }).each(["a", "b"]).ask({ "item1.answer": "Relevant?", "": "Done?" });
  assert.deepEqual(rows, [ { "item1.answer": true, "": true }, { "item1.answer": true, "": true } ]);
  assert.equal(calls.length, 1);
...
test("pre-aborted operations do not read live context or call the provider", async () => {
  ...
  const q = Questions.create({ model }).about(() => { reads++; return "context"; });
  await assert.rejects(q.is("Ready?", { signal: AbortSignal.abort(stop) }), (error) => error === stop);
...
test("cancellation settles a non-cooperative provider and preserves the reason", async () => {
  ...
  const stop = new Error("stop");
  const q = Questions.create({ model: { name: "slow", evaluate: () => new Promise(() => {}) } }).about("x");
...
test("question definitions snapshot inputs and handle prototype-like keys safely", async () => {
  ...
  const unusual = Object.fromEntries([["__proto__", "Safe?"], ["constructor", "Ready?"]]);
  ...
  assert.equal(values.__proto__, true); assert.equal(values.constructor, true);
});

test("invalid contexts fail before inference without silent JSON loss", async () => {
  const { model, calls } = fixture();
  const cyclic: { self?: unknown } = {}; cyclic.self = cyclic;
  for (const context of [undefined, NaN, new Date(), { bad: undefined }, { bad: 1n }, cyclic, [,,]]) {
    await assert.rejects(Questions.create({ model }).about(context as State).is("Ready?"), ValidationError);
  }
  assert.equal(calls.length, 0);
});
...
test("invalid definition cardinalities and missing criteria fail synchronously", () => {
  ...
  assert.throws(() => Question.score("How?", [] as unknown as readonly [string, string]), ValidationError);
